...
void test1()
{
    ...
    std::map<std::string, int> m{
        ...
        {"Moe", 1},
        {"Curly", 2}};
    display(m); //! sorted according to names and insertion also takes place in thsi form
    ...
    m["Frank"] = 18; //! insert the Frank and with value 18
    display(m);

    m["Frank"] += 10; //! find Frank and add 10 to it's value
    display(m);
    ...
    if (it != m.end())
        std::cout << "Found: " << it->first << " : " << it->second << std::endl; //! first will return the key and second will return value of that key.
...
void test2()
{
    ...
    display(grades); //! sorted in order of the alphabets and grades will also be in sorted form and insertion also takes place in this manner in sorted order.
