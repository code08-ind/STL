...
void test2()
{
    ...
    it = stooges.find(Person("XXXX", 50)); //! Will remove James as it's age matches with the 50 as of James
                                           // uses operator<
    if (it != stooges.end())
        stooges.erase(it);
    display(stooges);
...
void test3()
{
    ...
    display(s);
    result = s.insert("E");//! as E was not there, so it can be inserted in set so it returns true else if it was present before hand in the set so it will return false.
    display(s);
