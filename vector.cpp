...
void test2()
{
    ...
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl; //! capacity will double at each instance of the multiplier of size of vector that is if it's size was 5 then it will become 10 20 40 80 and so on ...

    vec.shrink_to_fit(); // C++11//? reserves max capacity to it's size.
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;

    vec.reserve(100); //? reserves max capacity to 100
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;
...
void test4()
{
    ...
    stooges.emplace_back("Curly", 30); // Use emplace_back!!!//? as it do same work more efficiently
    display(stooges);
}
...
void test6()
{
    ...
    display(vec);
    vec.erase(vec.begin(), vec.begin() + 2); //? delete that item which occurs at first and second excluding the third element
    display(vec);
...
void test9()
{
    ...
    std::copy(vec1.begin(), vec1.end(), std::back_inserter(vec2)); //! inserts elmenents of vec1 at end of vec2 and changes vec 2
    display(vec1);
    display(vec2);
    std::cout << std::endl;
